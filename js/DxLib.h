...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x3180
#define DXLIB_VERSION_STR_T _T( "3.18 " )
#define DXLIB_VERSION_STR_W    L"3.18 "
...
#define DX_MATERIAL_TYPE_MAT_SPEC_LUMINANCE_TWO_COLOR		(3)		// マテリアルのスペキュラ色の輝度が指定の閾値以上か、未満かで２色に塗り分けるマテリアル
#define DX_MATERIAL_TYPE_MAT_SPEC_POWER_TWO_COLOR			(4)		// マテリアルのスペキュラハイライトの鮮明度が指定の閾値以上か、未満かで２色に塗り分けるマテリアル
...
extern	int			InputStringToCustom(		int x, int y, size_t BufLength, TCHAR *StrBuffer, int CancelValidFlag, int SingleCharOnlyFlag, int NumCharOnlyFlag, int DoubleCharOnlyFlag = FALSE ) ;		// 文字列の入力取得
...
extern	int			MV1SetMaterialTypeParamAll(			int MHandle,                    float TypeParam ) ;						// 全てのマテリアルのタイプ別パラメータを変更する( マテリアルタイプ DX_MATERIAL_TYPE_MAT_SPEC_LUMINANCE_TWO_COLOR などで使用 )
extern	int			MV1SetMaterialTypeParam(			int MHandle, int MaterialIndex, float TypeParam ) ;						// 指定のマテリアルのタイプ別パラメータを変更する( マテリアルタイプ DX_MATERIAL_TYPE_MAT_SPEC_LUMINANCE_TWO_COLOR などで使用 )
extern	float		MV1GetMaterialTypeParam(			int MHandle, int MaterialIndex ) ;										// 指定のマテリアルのタイプ別パラメータを取得する
